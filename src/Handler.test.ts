...
import { response, get, lit, param } from './routeCombinators';
...
describe('test the handlers', () => {
  it('uses the healthz handler successfully', async () => {
    ...
    expect(result).toEqual(E.right(E.right({ code: 200, data: 'OK' })));
  });
  ...
  it('uses a doggy handler', async () => {
    ...
    const result = await runRouteWithHandler(dogAgesHandler)({
      url: '/dogs/100',
      method: 'get',
      rawData: {},
      rawHeaders: {},
    })();
    expect(result).toEqual(E.right(E.right({ code: 200, data: ['frank'] })));
  });
});
