...
describe('koa-router-fp-ts', () => {
  describe('testing with koa', () => {
      ...
      it('returns a 201 when the readyz route is found', async () => {
        await withServer(
          ...
          async (server) => {
            ...
            ).to.contain('text/plain; charset=utf-8')
    ...
    const userIdParam = param('userId', t.NumberFromString)
        ...
        await withServer(
          ...
          async (server) => {
            const reply = await request(server)
              .get('/user/dog')
              .expect(400)
            expect(reply.text).to.contain(
              'params: Invalid value of type string supplied to : { id: NumberFromString }/id: NumberFromString'
            )
          }
        )
      })
      ...
      it('returns a 200 when the route and headers match', async () => {
        await withServer(
          ...
          async (server) => {
            ...
            ).to.contain('application/json; charset=utf-8')
          }
        )
      })
    })
      ...
      it('returns json including Router metadata', async () => {
        await withServer(serve(router), async (server) => {
          const reply = await request(server)
            .get('/swagger2.json')
            .expect(200)
          expect(reply.body.info.description).toEqual(
            'Nice'
          )
        })
      })
