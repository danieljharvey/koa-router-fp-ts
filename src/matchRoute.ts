...
const parseQueryParams = (whole: string): Record<string, string> => {
  ...
  return flattenParams(as.map(([key, val]) => ({ [key]: val })));
};
...
const matchMethod = (
  ...
  pipe(
    method,
    O.map((m) => m.toLowerCase() === requestMethod.toLowerCase()),
...
const matchRouteItem = (
  ...
  if (routeItem.type === 'Literal') {
    return routeItem.literal.toLowerCase() === urlPart.toLowerCase()
      ? E.right({})
      : E.left(noMatch(`${urlPart} did not match ${routeItem.literal}`));
  }
...
): Record<string, string> => params.reduce((all, a) => ({ ...all, ...a }), {});
...
export const matchRoute =
  <ResponseInput, ResponseOutput, Param, Query, Data, Headers>(
  ...
  > => {
    ...
    if (pairs.length !== route.parts.length || pairs.length !== items.length) {
      return E.left(noMatch('Route does not match url parts length'));
    }

    if (!matchMethod(route.method, method)) {
      return E.left(noMatch(`Method does not match ${route.method}`));
    }
    ...
    return pipe(
      sequenceT(paramMatches, queryMatches, dataMatches, headersMatches),
