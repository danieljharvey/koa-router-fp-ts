...
const checkAuth = <Input extends { headers: { session: number } }>(
  input: Input,
): TE.TaskEither<NotAuthResponse, Input & AuthUserDetails> => {
...
const getUserRoute = makeRoute(
  ...
  response(t.union([userResponse, userNotFoundResponse, notAuthResponse])),
);
...
const getUsersHandler = ({
  ...
  HandlerInput<typeof getUsersRoute>): T.Task<UsersResponse> => {
