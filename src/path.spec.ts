  ...
  describe('path', () => {
    ...
    describe('has only interpolated items', () => {
      ...
      it('one item', () => {
        ...
        expect(result1.parts).to.have.length(1)
      ...
      it('two items', () => {
        ...
        expect(result2.parts).to.have.length(2)
        expect(result2.parts).toEqual(expected.parts)
        expect(
          JSON.stringify(result2.paramDecoder)
        ).toEqual(JSON.stringify(expected.paramDecoder))
        expect(areTypeEqual(result2, expected)).toEqual(
          true
        )
      })
      ...
      it('two items, no slash', () => {
        ...
        expect(result3.parts).to.have.length(2)
        expect(result3.parts).toEqual(expected.parts)
        expect(
          JSON.stringify(result3.paramDecoder)
        ).toEqual(JSON.stringify(expected.paramDecoder))
        expect(areTypeEqual(result3, expected)).toEqual(
          true
        )
      })
    })
